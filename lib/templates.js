...
export function getTemplateDefinition(key) {
  const defs = {
    ...
    keywordRoiCalculator: {
      ...
      fields: [
        { name: 'cpc', label: 'Cost Per Click ($)', type: 'number', placeholder: '2.50' },
        { name: 'conversion_rate', label: 'Conversion Rate (%)', type: 'number', placeholder: '2.0' },
        { name: 'value', label: 'Customer Value ($)', type: 'number', placeholder: '100' }
      ]
    },
  ...
  switch (key) {
    ...
    case 'seoContentChecker': {
      ...
      const examples = [
        ...
        'Poor Title: Best Content | Click Here | #1 SEO Checker 2025',
    ...
    case 'contentFreshnessChecker': {
      ...
      if (!hasRecentYears) problems.push('Missing recent year references (2024–2025).');

      const fixes = [
        'Update statistics and examples to 2024–2025 sources.',
      ...
      const improvedTitle = title || 'Content Freshness Checker: Keep Pages Up‑to‑Date in 2025';
      const improvedMeta = 'Evaluate and update on‑page content for 2025. Identify outdated references, improve readability, and optimize SEO signals for better rankings and engagement.';
      const improvedContent = [
        'Use this checklist to keep your content fresh and relevant in 2025.',
