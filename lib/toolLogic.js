...
export function runTemplate(key, inputs) {
  switch (key) {
    ...
    case 'metaTagGenerator': {
      ...
      const titleStatus = titleLen ? (titleLen >= 50 && titleLen <= 60 ? 'Good' : 'Adjust to 50â€“60 chars') : 'Missing';
      const descStatus = descLen ? (descLen >= 120 && descLen <= 160 ? 'Good' : 'Adjust to 120â€“160 chars') : 'Missing';
    ...
    case 'robotsTxtValidator': {
      ...
      const status = errors.length ? 'ðŸ”´ Invalid' : warnings.length ? 'ðŸŸ¡ Valid with Warnings' : 'ðŸŸ¢ Valid';
      ...
      return [
        ...
        '',
        '## Validation Details',
        ...(errors.length ? ['### âŒ Errors (Must Fix)', ...errors.map(e => `- ${e}`), ''] : []),
        ...(warnings.length ? ['### âš ï¸ Warnings (Best Practices)', ...warnings.map(w => `- ${w}`), ''] : []),
        (!errors.length && !warnings.length) ? 'âœ… No syntax errors or warnings detected.' : '',
        '',
        '## Analysis',
    ...
    case 'titleMetaLengthCounter': {
      ...
      return `Title: ${title.length} chars (50â€“60 recommended)\nMeta Description: ${desc.length} chars (120â€“160 recommended)`;
    }
    case 'seoContentChecker': {
      ...
      if (titleLenStatus === 'Adjust') { problems.push('Title length not ideal.'); fixes.push('Aim for â‰ˆ50â€“60 characters balancing keyword and value.'); }
      ...
      if (!meta) { problems.push('Missing meta description.'); fixes.push('Add a compelling meta description â‰ˆ120â€“160 characters.'); }
      ...
      if (!placedEarly && focus) { problems.push('Keyword not placed early.'); fixes.push('Mention the keyword in the opening paragraph or first 100â€“150 words.'); }
      if (readability < 55) { problems.push('Low readability.'); fixes.push('Shorten sentences, use active voice, and add bullets.'); }
      if (totalWords < 800) { problems.push('Content may be thin.'); fixes.push('Increase depth with examples, FAQs, and internal links (800â€“1200 words).'); }

      const examples = [
        'Optimized Title: SEO Content Checker â€“ Analyze Density, Readability, and Placement',
      ...
      const lines = [];
      lines.push('SEO Content Checker â€“ Results');
    ...
    case 'contentFreshnessChecker': {
      ...
      const problems = [];
      if (hasOldYears) problems.push('Outdated year references detected (2010â€“2022).');
      if (outdatedHits) problems.push('Old techniques mentioned (e.g., AMP, meta keywords, PageRank).');
      if (wordCount < 600) problems.push('Content may be thin; increase depth to 800â€“1200 words.');
      ...
      if (!hasRecentYears) problems.push('Missing recent year references (2024â€“2026).');

      const fixes = [
        'Update statistics and examples to 2024â€“2026 sources.',
        ...
        'Add â€œLast updatedâ€ date and ensure canonical signals are present.'
      ];

      const improvedTitle = title || 'Content Freshness Checker: Keep Pages Upâ€‘toâ€‘Date in 2026';
      const improvedMeta = 'Evaluate and update onâ€‘page content for 2026. Identify outdated references, improve readability, and optimize SEO signals for better rankings and engagement.';
      const improvedContent = [
        ...
        'Replace outdated techniques with current best practices (Eâ€‘Eâ€‘Aâ€‘T, internal linking, performant pages).'
      ].join('\n');
      ...
      return [
        `Freshness Score (0â€“100): ${freshnessScore}`,
        `SEO Score (0â€“100): ${seoScore}`,
        'Problems Found:',
    ...
    case 'aiArticleLengthOptimizer': {
      ...
      // Section heuristic: ~150â€“250 words per section, tune by content type
      ...
      const redundant = [...bigramCounts.entries()].filter(([, c]) => c >= 3).slice(0, 10).map(([bg, c]) => `${bg} Ã—${c}`);
      ...
      const expansions = foundAbbrevs.map((k) => `${k.toUpperCase()} â†’ ${abbrevMap[k]}`);

      const outline = [
        'Introduction (80â€“120 words)',
        'Key Concepts (200â€“300 words)',
        'Step-by-Step/How-To (400â€“600 words)',
        'Examples/Use Cases (250â€“400 words)',
        'Common Pitfalls (150â€“250 words)',
        'Advanced Tips (200â€“300 words)',
        'Conclusion & Next Steps (100â€“180 words)'
      ];

      const tips = [
        'Use subheadings every 150â€“250 words and keep paragraphs short.',
      ...
      return [
        ...
        '',
        `Recommended Length: ${recommended} words (95% CI: ${ci95[0]}â€“${ci95[1]})`,
...
AI Indicators Found:
${aiIndicators.length > 0 ? aiIndicators.map(i => `â€¢ ${i}`).join('\n') : 'â€¢ None detected'}

Human Indicators Found:
${humanIndicators.length > 0 ? humanIndicators.map(i => `â€¢ ${i}`).join('\n') : 'â€¢ None detected'}
...
`;
    ...
    case 'siteComparisonReportGenerator': {
      ...
      const tableRows = metrics.map(m => {
        ...
        const w1 = val1 > val2 ? 'ðŸ†' : '';
        const w2 = val2 > val1 ? 'ðŸ†' : '';
    ...
    case 'keywordComparisonTool': {
      const listA = (inputs.listA || '').split('\n').map(k => k.trim().toLowerCase()).filter(Boolean);
      const listB = (inputs.listB || '').split('\n').map(k => k.trim().toLowerCase()).filter(Boolean);

      if (!listA.length && !listB.length) return 'Please enter keywords in List A and List B.';

      const setA = new Set(listA);
      const setB = new Set(listB);

      const intersection = listA.filter(k => setB.has(k)).filter((v, i, a) => a.indexOf(v) === i); // Unique intersection
      const uniqueA = listA.filter(k => !setB.has(k)).filter((v, i, a) => a.indexOf(v) === i);
      const uniqueB = listB.filter(k => !setA.has(k)).filter((v, i, a) => a.indexOf(v) === i);
      ...
      return [
        ...
        `| Metric | Count |`,
        `| :--- | :--- |`,
        `| **List A Total** | ${listA.length} |`,
        `| **List B Total** | ${listB.length} |`,
        `| **Shared Keywords** | ${intersection.length} |`,
        `| **Unique to A** | ${uniqueA.length} |`,
        `| **Unique to B** | ${uniqueB.length} |`,
        '',
        '## Shared Keywords (Intersection)',
        intersection.length ? intersection.map(k => `- ${k}`).join('\n') : '*No shared keywords found*',
        '',
        '## Unique to List A',
        uniqueA.length ? uniqueA.map(k => `- ${k}`).join('\n') : '*No unique keywords in List A*',
        '',
        '## Unique to List B',
        uniqueB.length ? uniqueB.map(k => `- ${k}`).join('\n') : '*No unique keywords in List B*'
      ].join('\n');
    }
    ...
    case 'reviewResponseGenerator': {
      ...
      if (isPositive) {
        ...
        responses.push({
          style: 'Warm & Personal',
          text: `Hi ${name}! Thanks for the love! ðŸŒŸ We're thrilled you enjoyed your visit. Knowing that you appreciated ${pivot} really makes our day. Can't wait to see you again!`
        });
        responses.push({
          style: 'Short & Sweet',
