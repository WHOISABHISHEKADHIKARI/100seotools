...
export function runTemplate(key, inputs) {
  switch (key) {
    ...
    case 'seoContentChecker': {
      ...
      if (titleLenStatus === 'Adjust') { problems.push('Title length not ideal.'); fixes.push('Aim for â‰ˆ50–60 characters balancing keyword and value.'); }
      ...
      if (!meta) { problems.push('Missing meta description.'); fixes.push('Add a compelling meta description â‰ˆ120–160 characters.'); }
...
`;
    ...
    case 'featuredToolSchemaGenerator': {
      ...
      const pageUrl = inputs.pageUrl || `${baseUrl}/tool-url`;
      const canonicalUrl = pageUrl;
      const brandLogo = inputs.brandLogo || `${baseUrl}/logo.png`;
