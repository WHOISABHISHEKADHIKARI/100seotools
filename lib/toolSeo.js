...
function clickableToolTitle(tool = {}) {
  const name = baseToolName(tool);
  const short = shortenToolName(name);
  const outcome = outcomeForTool(tool);
  const brand = '100SEOTools';

  // Prefer benefit-driven format: "{Name} — Free, No Login | 100SEOTools"
  const primary = `${short} — Free, No Login | ${brand}`;
  if (primary.length <= 60) return primary;

  // Medium: "{Name} — Free, No Login"
  const medium = `${short} — Free, No Login`;
  if (medium.length <= 60) return medium;

  // Short: "Free {Name} - {Outcome}"
  const fallback = `Free ${short} - ${outcome}`;
  return fallback;
}

export function makeToolSeoTitle(tool = {}) {
  if (tool.metaTitle) return squeezeTitle(tool.metaTitle);
  return squeezeTitle(clickableToolTitle(tool));
}
