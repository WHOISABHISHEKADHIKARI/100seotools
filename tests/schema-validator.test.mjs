 ...
 * Tests JSON-LD structured data across all page types
 * Run with: node tests/schema-validator.test.mjs
 */
...
const TEST_PAGES = [
    ...
    },
    {
        ...
        name: 'Tool Page',
        expectedSchemas: ['SoftwareApplication', 'WebPage', 'BreadcrumbList', 'HowTo'],
        requiredProperties: {
    ...
    },
    {
        url: '/blog/seo-basics',
        name: 'Blog Post',
        expectedSchemas: ['Article', 'WebPage'],
        requiredProperties: {
 ...
 */
function validateProperties(schema, requiredProps, schemaType) {
    const missing = [];
        ...
        Object.entries(obj).forEach(([key, value]) => {
            if ((key === 'url' || key === 'item' || key === '@id') && typeof value === 'string') {
                if (!value.startsWith('http://') && !value.startsWith('https://')) {
                    issues.push(`Relative URL found at ${path}.${key}: ${value}`);
                }
            }
...
async function testPage(pageConfig) {
    ...
    try {
            ...
            if (!schema) {
                ...
                if (pageConfig.requiredProperties && pageConfig.requiredProperties[expectedType]) {
                    ...
                    if (missing.length > 0) {
                        totalTests++;
                        failedTests++;
                        pageErrors.push(`${expectedType} missing properties: ${missing.join(', ')}`);
        ...
        // Validate URLs
        const urlIssues = validateUrls(schemas);
        if (urlIssues.length > 0) {
            totalTests++;
            failedTests++;
            pageErrors.push(...urlIssues);
            ...
            totalTests++;
            passedTests++;
            console.log(`${colors.green}✓ All URLs are absolute${colors.reset}`);
        }
