    ...
    return (
        <main id="main" className="container mx-auto px-4 py-8">
            ...
            <header className="mb-8">
                <h1 className="text-3xl md:text-4xl font-bold mb-4">All SEO Tools</h1>
                <p className="text-lg text-slate-600 dark:text-slate-300 max-w-3xl">
                    Browse our complete collection of {tools.length}+ free SEO tools. All tools are 100% free, require no signup, and provide instant results.
                </p>
            </header>

            {/* Category-based tool listing */}
            <div className="space-y-10">
                {Object.entries(categories).sort(([a], [b]) => a.localeCompare(b)).map(([category, categoryTools]) => (
                    <section key={category} aria-labelledby={`cat-${category.replace(/\s+/g, '-').toLowerCase()}`}>
                        ...
                        >
                            {category} <span className="text-base font-normal text-slate-500">({categoryTools.length} tools)</span>
                        </h2>
                        <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-4">
                            {categoryTools.map((tool) => (
                                <Link
                                    ...
                                    prefetch={false}
                                    className="block p-4 rounded-lg border border-slate-200 dark:border-white/10 bg-white dark:bg-slate-900/40 hover:shadow-md hover:border-brand-500 dark:hover:border-brand-400 transition-all"
                                >
                                    <h3 className="font-medium text-slate-900 dark:text-white mb-1">{tool.name}</h3>
                                    <p className="text-sm text-slate-600 dark:text-slate-300 line-clamp-2">{tool.description}</p>
                                </Link>
                            ))}
                        </div>
                    </section>
