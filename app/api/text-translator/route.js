...
export async function POST(request) {
    try {
        ...
        const targetLanguage = typeof target_lang === 'string' && target_lang.trim()
            ? target_lang.trim()
            : 'spanish';
        const langMap = {
        ...
        const langCode = langMap[targetLanguage.toLowerCase()] || targetLanguage.toLowerCase();
        ...
        output += `Open this Google Translate link for ${targetLanguage}:\n${translateUrl}\n\n`;

        output += `### Alternative (DeepL)\n`;
        output += `For higher nuance, try DeepL:\n`;
        output += `https://www.deepl.com/translator#en/${langCode}/${encText}`;
