...
export default function HomePage() {
  ...
  useEffect(() => {
    // Use the dedicated tool metadata function to ensure only tools are listed
    const onlyTools = getAllToolsMeta().map(t => ({ ...t, type: 'tool' }));
    console.log('Main Page Tools Count:', onlyTools.length);
    setTools(onlyTools);
    setFilteredTools(onlyTools);
    setIsLoaded(true);
  }, []);
