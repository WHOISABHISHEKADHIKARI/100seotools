...
export default function HomePage() {
  ...
  useEffect(() => {
    const liveTools = getAllToolsMeta().map((tool) => ({ ...tool, type: 'tool' }));
    setTools(liveTools);
    setFilteredTools(liveTools);
  }, []);
  ...
  const webPageLd = {
    ...
    description: 'Complete collection of 100+ free SEO tools for keyword research, on-page optimization, technical SEO, content analysis, and performance tracking.',
    url: baseUrl,
    keywords: ['100 SEO tools', 'free SEO tools list', 'SEO tool comparison', 'free SEO toolkit'],
  };
